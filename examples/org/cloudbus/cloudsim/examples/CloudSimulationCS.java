...
public class CloudSimulationCS {
    ...
    public static void main(String[] args) {
        ...
        Log.printLine("Starting Cloud Simulation with CS...");
...
//  		    int cloudletNumber = bot*1000;
            ...
            for (int cloudletIterator = 0; cloudletIterator <= cloudletLoopingNumber; cloudletIterator++) {
                ...
                // Looping untuk setiap data center
                for (int dataCenterIterator = 1; dataCenterIterator <= 6; dataCenterIterator++) {
                    System.out.println("Datacenter " + dataCenterIterator);

                    // Parameter untuk Cuckoo Search
                    int maxIterations = 5; // Jumlah iterasi maksimum
                    int populationSize = 30; // Jumlah sarang (nests)
                    double discoveryRate = 0.25; // Probabilitas penemuan telur alien

                    // Inisialisasi Cuckoo Search
                    CuckooSearch cs = new CuckooSearch(maxIterations, populationSize, discoveryRate, cloudletList, vmlist);

                    // Jalankan Cuckoo Search
                    cs.runCS(cloudletNumber, dataCenterIterator, cloudletIterator, CuckooSearch::sixHumpCamelBack);
                    ...
                    while (iteration <= maxIterations) {
                        ...
                        System.out.println("Iteration " + iteration + " Best Fitness for DC" + dataCenterIterator
                                + ": " + cs.getBestFitnessForDatacenter(dataCenterIterator));
                        iteration++;
                    }

                    // Dapatkan solusi terbaik
                    int[] bestSolution = cs.getBestVmAllocationForDatacenter(dataCenterIterator);
...
//                        System.out.println("Assigner: " + assigner + " vmId: " + vmId);
                         broker.bindCloudletToVm(assigner, vmId); // Uncomment jika menggunakan broker
                    }
                }
            }
            ...
            Log.printLine("Cloud Simulation with CS finished!");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static double rosenbrocksValley(double[] variablesValues) {
        double funcValue = 0;
        double lastX = variablesValues[0];
        for (int i = 1; i < variablesValues.length; i++) {
            funcValue += (100 * Math.pow((variablesValues[i] - Math.pow(lastX, 2)), 2)) + Math.pow(1 - lastX, 2);
        }
        return funcValue;
    }
